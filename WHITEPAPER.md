...
**The oracle half** is consumed in four places, which is why it cannot be patched out locally: the index defines truth; the funding rate taxes the gap between the venue's mark and that index, continuously; margin checks value positions at it; and forced closes settle at it. Each consumption point inherits the feed's latency, its manipulation surface — located at its *sources*, outside the protocol entirely — and its listing bottleneck. The failure is not hypothetical. In September 2022, a trader ran five cycles of ~$4–5M positions through GMX's zero-slippage, oracle-priced entry while pushing the AVAX price on the comparatively thin exchanges feeding the oracle, extracting ~$565k from the protocol's LPs [1, 2]. The lesson generalizes: **an oracle-priced door with no price impact converts source manipulation into a free option against whoever takes the other side.**

**The event half** creates reflexivity and an unpriceable tail. A one-shot liquidation dumps a position at the worst moment; the sale moves the price; the move breaches the next trader — the cascade dynamic marking every major deleveraging. Behind the event sit backstops of last resort: insurance funds (Perpetual Protocol v1's drained under sustained skew and a single chaotic liquidation, halting the protocol [3, 4]) or socialized auto-deleveraging, in which the *most profitable opposite-side traders* are force-closed against the bankrupt account — formalized recently as unavoidably unfair to winners [5]. Hyperliquid's March 2025 JELLY incident showed the two halves failing together: its backstop vault *inherits* liquidated positions, so an attacker self-liquidated a $4–5M short into it, pumped the token on external venues, and put the house $13.5M underwater — resolved only by a validator vote to delist and force-settle at a chosen price [6, 7]. When the backstop of last resort turns out to be governance, the tail was never priced; it was postponed.
...
settled lazily per position against a snapshot. The crowded side pays, the thin side receives, and the residual $|\dot f| \cdot |L - S|$ — the carry on exposure no trader offsets — accrues to the vault that carries it. Two precedents locate the default $k = 100\%/\mathrm{yr}$-at-full-skew: CEX convention (baseline 0.01%/8h, with Hyperliquid capping its premium rate at 4%/hour) brackets it from both sides [11], and Perpetual Protocol v1 marks the failure mode this form avoids — a vAMM has no natural funding payer, so its insurance fund paid the crowded side until it drained [3]; here the residual flows *to* the house, never from it. Synthetix v2's skew-*velocity* refinement (the rate drifts while imbalance persists) is the documented upgrade path [10].
...
Oracle-consuming perps refine the same template — dYdX and Hyperliquid at the orderbook end (the latter's liquidation waterfall, backstop vault, and socialized ADL are the genre's cleanest [11, 12]); GMX at the LP-pool end, whose zero-impact oracle door was exploited in 2022 [1, 2] and whose v2 fee structure has converged toward skew-priced carry; Synthetix, whose skew funding and impact functions keep its markets near-neutral [9, 10]; Perpetual Protocol v1, whose vAMM showed that marks need real inventory and funding needs a real payer [3, 4]. The oracle-free family before TruePerp avoids liquidation by pre-paying it: InfinityPools borrows Uniswap v3 ranges so the worst case is funded upfront (oracle-free, liquidation-free, capital-heavy) [13]; Numoen sells fully-collateralized perpetual options [14]; Contango synthesizes perps by looping through money markets, inheriting their oracles and liquidation events wholesale [15]. TruePerp occupies the remaining cell: liquidation neither avoided by pre-payment nor inherited from an oracle venue, but performed — gradually, in cash, by the settlement venue itself — on the kernel introduced for oracleless lending by TrueLend [8], whose gradual-conversion ancestry (Curve's LLAMMA) and internal-median pricing (Panoptic) are documented there. Perp-oriented v4 hooks exist today as early-stage projects without production liquidation mechanisms [16]; the kernel is precisely the missing piece.
